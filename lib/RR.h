 ...
 * @result a pointer to the new running process (if something changed) or to the
 ...
 * @param quantum the specified (last parameter in program call) or the default
 * quantum
 ...
 * @result A pointer to the new active (running) process (if that changed) or to
 * the previously running process
 */
process *RR_new_arrival(process *arriving_process, process *running_process);

/**
 * is called once after the all processes were handled. In case you want to
 * cleanup something
 */
void RR_finish();
#endif  // RR_H
