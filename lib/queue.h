...
/**
 * adds a new queue object right behind the very first element in the queue
 * (@param queue remains the head of the queue).
 *
 * Remember to allocate space for a new queue object and to adjust the pointers
 ...
 *
 * Remember to properly check for NULL-values before dereferencing pointers and
 * to free removed queue object. Also make sure to adjust pointers correctly.
 *
 * @param queue The pointer to a queue handler you want to poll from
 * @result pointer to an object that was previously added using function int
 * queue_add or NULL if the queue is empty
 */
void *queue_poll(queue_object *queue);

/**
 * Creates and returns a new queue. Should be called before working with the
 * queue.
 *
 * A new queue consists of a single queue object which is used as a head
 * element. Remember to allocate enough space for any object you need to create.
 *
 ...
 * @result pointer to an object that was previously added using queue add or
