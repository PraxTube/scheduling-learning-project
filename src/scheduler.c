...
static void print_and_save_process(process *running_process,
                                   unsigned int time) {
...
unsigned int end_time(unsigned int p_id) {
    unsigned int local_end = arriving_processes[p_id]->start_time +
...
// checks for newly arriving processes at current _time_. Return NULL if there
// is none
static process *check_new_arrival(unsigned int process_count,
                                  unsigned int time) {
    ...
    switch (strategy) {
        ...
        case PRIOP:
            ...
            for (unsigned int time = 0; time < max_time; time++) {
                ...
                running_process =
                    PRIOP_new_arrival(arriving_process, running_process);
        ...
        case SRTN:
            ...
            if (SRTN_startup() != 0) {
                fprintf(stderr, "Problem starting the SRTN scheduler\n");
                exit(1);
            }
            for (unsigned int time = 0; time < max_time; time++) {
                // check for new arrival at given point of time
